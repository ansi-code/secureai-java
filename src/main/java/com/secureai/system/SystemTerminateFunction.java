...
public class SystemTerminateFunction implements TerminateFunction<SystemState> {
    ...
    public boolean terminated(SystemState systemState) {
        for (String resourceId : this.environment.getSystemDefinition().getResources())
            if (!systemState.get(resourceId, State.active) || !systemState.get(resourceId, State.updated) || systemState.get(resourceId, State.upgradable) || systemState.get(resourceId, State.vulnerable) || systemState.get(resourceId, State.corrupted))
                return false;
